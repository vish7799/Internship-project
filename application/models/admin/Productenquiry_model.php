...
class Productenquiry_model extends CI_Model {
	...
	function get_productenquiry($page = 1){
		$start = ($page-1) * get_setting('limit');
		$end = get_setting('limit');
		$limit = " LIMIT $start, $end";
		$q = "SELECT product_inquiry.* FROM product_inquiry order by product_inquiry.id desc" . $limit;
	...
	public function get_all_pages_count(){
		$q = "SELECT count(*) as count FROM product_inquiry order by product_inquiry.id";		
		$query = $this->db->query($q);
		return $query->row()->count;
	}
	
	public function get_page_info($page_id){
		$q = "SELECT * FROM product_inquiry where id = ". $page_id;
		$query = $this->db->query($q);
		return $query->row();
	}
	
	function add_productenquiry($post){
		$post['created'] = time();
		$this->db->insert('product_inquiry',$post);
		$productenquiry_id = $this->db->insert_id();
		return $productenquiry_id;
	}

	function update_productenquiry($page_id,$post){
		$this->db->where('id',$page_id);
		$this->db->update('product_inquiry',$post);
		return true;
	}
